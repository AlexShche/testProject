...
export const Prods = () => {
    ...
    const [prods] = useState(data)
    const [activeTabsProds, setActiveTabsProds] = useState([])

    const handleTab = (index, def = false) => {
        if (def) {
            setActiveTabsProds([...activeTabsProds].filter(i => i !== index))
        } else if (!activeTabsProds.includes(index)) {
            setActiveTabsProds(
                [...activeTabsProds, index]
            )
        }
    }
                ...
                prods.map((prod, i) => {
                    return (
                        <div key={i} className='prod'>
                            <Row gutter={10}>
                                <Col lg={6}>
                                    <img className='prodsImg' src={prod.primaryImageUrl} alt=""/>
                                </Col>
                                <Col lg={10}>
                                    <div>
                                        <span className='code'>Код: {prod.code}</span>
                                        <span
                                            className={`${prod.isActive ? 'inStock' : 'none'} status`}>{prod.isActive ? 'Наличие' : 'Нет в наличие'}</span>
                                    </div>
                                    <p className='title'>
                                        {prod.title}
                                    </p>
                                ...
                                <Col lg={8} className='buyInfo'>
                                    <div className='price'>
                                        <span className='titlePrice'>По карте клуба: </span>
                                        <span
                                            className={`${activeTabsProds.includes(i) ? 'disabled' : ''} priceBuy`}
                                        >
                                                {prod.priceGoldAlt} ₽
                                            </span>
                                        <div
                                            className={!activeTabsProds.includes(i) ? 'disabled' : ''}
                                        >
                                            {prod.priceGold} ₽
                                        </div>
                                    </div>
                                    <div className='bonus'>
                                        {!prod.bonusAmount && 'Бонусов нет'}
                                    </div>
                                    <div className='themeTabsType'>
                                            <span
                                                className={`${!activeTabsProds.includes(i) && 'active'}`}
                                                onClick={() => handleTab(i, true)}
                                            >
                                                За м.кв.
                                            </span>
                                        <span
                                    ...
                                    <Row gutter={6} className='buyBtn'>
                                        ...
                                        <Col span={18}>
                                            <Button data-product-id={prod.id}>
                                                <svg viewBox="0 -31 512.00026 512">
                                                    <path
